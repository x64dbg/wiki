...
**Q**: Why is ARM64 emulation not supported?

**A**: The ARM64 emulation layer works by translating the application's x86/x64 code to ARM64 at runtime. The system libraries are still only in ARM64, which is not supported by x64dbg. While some operations work, you will not be able to step through system libraries and sometimes you will completely lose control of the execution (breakpoints are not hit and stepping will just run the application). This is an artifact of the translation process and cannot be solved except by rewriting x64dbg to support ARM64 properly.
